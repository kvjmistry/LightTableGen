#!/bin/bash
#SBATCH -J NEW_PSF # A single job name for the array
#SBATCH -c 1 # Number of cores
#SBATCH -p shared # Partition
#SBATCH --mem 4000 # Memory request (6Gb)
#SBATCH -t 0-12:00 # Maximum execution time (D-HH:MM)
#SBATCH -o NEW_PSF_%A_%a.out # Standard output
#SBATCH -e NEW_PSF_%A_%a.err # Standard error
...
N_PHOTONS=1000000
N_EVENTS=100
CONFIG=NEW_PSF.config.mac
INIT=NEW_PSF.init.mac
...
# Copy the files over
cp ~/packages/LightTableGen/config/NEW_PSF* .

# Change the output file na,e
sed -i "s#.*outputFile.*#/nexus/persistency/outputFile NEW_PSF_${SLURM_ARRAY_TASK_ID}.next#" ${CONFIG}
